...
16. WarnungKeine Restaurierung unbewerteter Chromosome, wenn mehr 
    ...
    wurden.                                                          WJ 14.02.19
17. Neue Variable "defaultChrFileMode", die bei gesetztem Schalter 
    WITH_DYN_GENES auf FILE_TXT_IO gesetzt wird. Sonst hat sie den 
    Wert FILE_BIN_IO. Bei FILE_BIN_IO wird bei den interaktiven Vari-
    anten bei Einzel-File-I/O der Filetyp erfragt, bei EXP-File-IO 
    wird hingegen der Wert von "defaultChrFileMode" verwendet. 
    Anpassung von read_elem() und write_elem() derart, dass nur soviel 
    Genparameter geschrieben werden, wie das Genmodell vorsieht.     WJ 28.06.19
18. WITH_DYN_GENES: read_elem(): Additional optional code for the 
    case of genes with unlimited number of parameters.
    read_chain(): Error handling according to flexible gene sizes.   WJ 08.07.19
19. restore_chs(): Avoiding buffer overflow of static strings. Some
    minor improvements in reporting.                                 WJ 06.08.20
--------------------------------------------------------------------------------
Status: o.k.                     Autor: W.Jakob                Datum: 06.08.2020
================================================================================ */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
INT  defaultChrFileMode;      /* Default-Fileformat der Chromosomendatei      */
      ...
      case FILE_TXT_IO:
         ...
         { /* Elem */
            ...
            switch (elem_ptr->u_el.activity)
            {
               ...
               default: /* Aktion */
		  get_act_descr (&act_descr_ptr, elem_ptr->u_el.activity, WITHOUT_GENECODE_CHECK);
                  for (ii = 0;  (ii < act_descr_ptr->i_par_anz) && ok;  ii++)
                     ok = fprintf (chain_file, "%d ", elem_ptr->u_el.u_xel.u_act.i_params[ii]) > 0;
                  if (ok)
                     ok = fprintf (chain_file, "  ") > 0;
                  for (ii = 0;  (ii < act_descr_ptr->r_par_anz) && ok;  ii++)
      ...
      case FILE_TXT_IO:
         ...
         { /* Elem */
		  ...
		  break; /* ACTIVITY_ACT */
                  ...
                  default: /* Aktion */
		     if (get_act_descr (&act_descr_ptr, elem_ptr->u_el.activity, 
					WITH_GENECODE_CHECK) == GLEAM_OK) /*creates err.msg.*/
		     { /* valid gene code */
#ifdef WITH_DYN_GENES /* with genes of unlimited number of parameters */
			ok = (initGene(elem_ptr, elem_ptr->u_el.activity) == GLEAM_OK);    
#endif /* with genes of unlimited number of parameters */
			for (ii = 0;  (ii < act_descr_ptr->i_par_anz) && ok;  ii++)
                 ...
                 BOOL                 reset_state)
{
   ...
   *ch_ptr      = NULL;
   ...
   { /* Header gelesen */
      ...
      { /* valid header */
         ...
         { /* valid chain context */
			...
			nextIstNull = (ptr->u_el.next_action == NULL);
			   ...
			   ptr->u_el.activity = 0;  /* zum sicheren Loeschen  */
                           free_chain_elem (ptr);   /* durch free_chain_elem()*/
                           ch_ok = FALSE;
                        } /* I/O-Error */
                        else 
                           if (ch_ok)
	 ...
	 chain_ptr->u_el.activity = 0;             /* zum sicheren Loeschen   */
         free_chain_elem (chain_ptr);              /* durch free_chain_elem() */
      } /* invalid header */
   } /* Header gelesen */
   else
...
BOOL chain_io_init (BOOL  mit_logging)
{
   ...
   defaultChrFileMode = FILE_TXT_IO;
#else
   defaultChrFileMode = FILE_BIN_IO;
#endif /* WITH_DYN_GENES */
   initialChrFileMode = defaultChrFileMode;
#ifdef MPI_PAR_VERS
   return (initMPI_Chio());
#else
   return (TRUE);
#endif /*f MPI_PAR_VERS */
} /* chain_io_init */
