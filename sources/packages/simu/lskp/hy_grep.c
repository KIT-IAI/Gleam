...
GLEAM/AE                                                                  V1.0.0
       ...
       Genetic LEarning Algorithms and Methods / Application Environment
                       ...
                       Package: SIMUlator/LeSaKPlus
                        Module: LeSaK-Genetic-REPair fuer HYgleam
 
Package: simu                   File: hy_grep.c               Unterpackage: lskp
--------------------------------------------------------------------------------
...
Diese Version ist auf den Mitsubishi mit einem Handlungsmodell mit Aktionen pro
Motor zugeschnitten. Fuer Experimente mit HyGLEAM.
Zu dieser Variante gehoeren: "robsimhy.c", "hy_grep.c" und "lskmodhy.h".
		   ...
		   GSTATE              *ret_state)
{
   ...
   { /* Plausibel, wenn mindestens ein Motor angeschaltet wird */
	 ...
	 motor_ein[ii] = FALSE;                    /* Init: Kein Motor ist an */
      ...
      { /* Pruefe diese Aktion */
	 geloescht    = FALSE;
	 ein_motor_an = ein_motor_an || 
	                   ((MOTOR_1 <= ptr->u_el.activity) && 
			    (ptr->u_el.activity <= MOTOR_5));
