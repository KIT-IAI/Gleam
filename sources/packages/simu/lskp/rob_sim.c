...
Aenderung gegenueber Version 1.0.0:
 1. "hole_obs" statt "load_obs"                                      WJ 09.12.98
 2. Anpassung der Aufrufschnitstelle von "hole_obs" an "dbr"         WJ 18.06.99
 3. "hindernismanagement" als noch nicht impl. Routine eingefuegt.   WJ 21.06.99
 4. Korrekturen wegen g++- und dann wegen bcc-Compiler.              WJ 12.08.00
 5. Neg. Rampenwerte werden zu 0.1 statt 0.0 korrigiert.  WJ 25.08.00 / 29.01.01
 6. Verschiebung von "write_to_simulog" nach "sys" / "file_gsw".     WJ 20.04.12
 7. Streichung der "#ifndef GLEAM_TCL"-Anweisungen. Aufruf von
    "write_text_buf" und "clr_line" in "eintakt" nur bei gesetztem 
    TEXT_MODE. Korrektur von Formatvorgaben. Entfernung ueber-
    fluessiger Variable.                                             WJ 27.06.12
 8. Renovierung: Korrektur der fscanf- und fgets-Aufrufe. Streichung 
 ...
 9. Anpassung des Aufrufs von "write_to_simu_log".                   WJ 14.09.17
10. Lokalisierung von JOINTS: Die Variablen "start_joints" und 
...
static BOOL pos_vergleich    (INT     vgl_op,
                              DOUBLE  vergleichswert,
                              INT     achse);

static BOOL geschw_vergleich (INT      vgl_op,
                              DOUBLE   vergleichswert,
                              INT      achse);

static BOOL beschl_vergleich (INT      vgl_op,
                              DOUBLE   vergleichswert,
                              INT      achse);

static void poscomp          (INT   achse);

static void geschwcomp       (INT   achse);

static void beschlcomp       (INT   achse);

static void zeitcomp         (void);

static void senscomp         (void);

static void abstandsvergl    (INT   switch_1);
   ...
   *robotframe = *anfangsframe;            /* Startwert des Roboterframes     */
   ...
   { /* Ergebnisdatenaufbereitung */
	 ...
	 if (nichtevo)
	 {
      ...
      if (kollision || (anschlag && anschlag_ko))
...
static void setzeu (INT  motor)
{
   ...
   { /* Falls Achse IN [1..axisanz] */
      r = act_item->u_el.u_xel.u_act.r_params[RAMPE];
      if (r < 0.0)
      { /* Rampe negativ ! */
	 r = 0.1;
	 fehler (SIMU_ROB_SIM, 9, NO_INFO, NO_INFO, BAD_RAMP_VAL_TXT, 
		 SIMU_RZ_MELD);               /* SIMU_RZ_MELD bringt nur Text */
#ifdef MITSIMULOG
	 write_to_simulog (BAD_RAMP_VAL_TXT, CLOSE_LOG);
#endif /* MITSIMULOG */
      } /* Rampe negativ ! */
      steuerfeld[motor].spannung = act_item->u_el.u_xel.u_act.r_params[U_WERT];
      steuerfeld[motor].rampe    = r;
#if defined MIT_ANIM || defined MITSIMULOG
      if (nichtevo)
      { /* interaktiver Aufruf */
	 sprintf (meld_buf, MOTOR_AN_LOG_TXT, motor, 
		  act_item->u_el.u_xel.u_act.r_params[U_WERT], r);
#ifdef MITSIMULOG
	 write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
...
static void motoraus (INT  motor)
{
   ...
   { /* Falls Achse IN [1..axisanz] */
	 ...
	 write_to_simulog (BAD_RAMP_VAL_TXT, CLOSE_LOG);
      ...
      if (nichtevo)
      { /* interaktiver Aufruf */
	 sprintf (meld_buf, MOTOR_AUS_LOG_TXT, motor, r);
#ifdef MITSIMULOG
	 write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
...
static void aktinterpreter (void)
{
   ...
   { /* Falls nicht NIL-Pointer */
	 ...
	 case SETZE_U_M:
	 setzeu (act_item->u_el.u_xel.u_act.i_params[MOTOR_NR]);
	 break;
        
	 case MOTOR_AUS:
	 motoraus (act_item->u_el.u_xel.u_act.i_params[MOTOR_NR]);
	 break;
        
	 case POS_VERGL:
	 poscomp (act_item->u_el.u_xel.u_act.i_params[GELENK_NR]);
	 break;
        
	 case GESCHW_VERGL:
	 geschwcomp (act_item->u_el.u_xel.u_act.i_params[GELENK_NR]);
	 break;
        
	 case BESCHL_VERGL:
	 beschlcomp (act_item->u_el.u_xel.u_act.i_params[GELENK_NR]);
	 break;
        
	 case ABSTAND_TCP:
	 if (mitr500)                          /* Abfrage, ob Mitsubishi R500 */
	    posktrwr500 (&pos, robotjoints);               /* fuer Mitsubishi */
	 else
	 {
	    ktrwrobmod (robotjoints, axisanz, OHNEORI);    /* fuer Robotermod.*/
	    pos = robotframe->transl;
	 }
	 distanz = vdist (&(zielframe->transl), &pos);
	 switch (act_item->u_el.u_xel.u_act.i_params[VERGL_OP])
	 {
	    case EQ:
            tcpvergl=act_item->u_el.u_xel.u_act.r_params[VERGL_WERT] == distanz;
            break;
	    case NE:
            tcpvergl=act_item->u_el.u_xel.u_act.r_params[VERGL_WERT] != distanz;
            break;
	    case GT:
            tcpvergl= distanz > act_item->u_el.u_xel.u_act.r_params[VERGL_WERT];
            break;
	    case LT:
            tcpvergl= distanz < act_item->u_el.u_xel.u_act.r_params[VERGL_WERT];
            break;
	    default:
	    fehler (SIMU_ROB_SIM, 12, NO_INFO, NO_INFO, BAD_POS_VGL_OP_TXT, 
		    SIMU_RZ_MELD);            /* SIMU_RZ_MELD bringt nur Text */
            tcpvergl = FALSE;
#ifdef MITSIMULOG
	    write_to_simulog (BAD_POS_VGL_OP_TXT, CLOSE_LOG);
#endif /* MITSIMULOG */
            break;
	 } /* CASE */
	 if (tcpvergl)
	    springanz = act_item->u_el.u_xel.u_act.i_params[AKT_ANZ];
#if defined MIT_ANIM || defined MITSIMULOG
	 if (nichtevo)
	 { /* interaktiver Aufruf */
	    compstr (buf, act_item->u_el.u_xel.u_act.i_params[VERGL_OP]);
	    sprintf (meld_buf, TCP_VERGL_LOG_TXT, distanz, buf,
		     act_item->u_el.u_xel.u_act.r_params[VERGL_WERT]);
#ifdef MITSIMULOG
	    write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
	 ...
	 case ABSTAND_GREIFER:
	 abstandsvergl (GREIFER);
	 break;
        
	 case ABSTAND_GREIFER_OUT:
	 abstandsvergl (GREIFEROUT);
	 break;
        
	 case DREHE_ABST_SENSOR:
	 sensordrehung = act_item->u_el.u_xel.u_act.r_params[GRAD];
#if defined MIT_ANIM || defined MITSIMULOG
	 if (nichtevo)
	 { /* interaktiver Aufruf */
	    sprintf (meld_buf, DREHE_ABST_SENS_LOGTXT, sensordrehung);
#ifdef MITSIMULOG
	    write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
	 ...
	 if (nichtevo)
	 { /* interaktiver Aufruf */
	    sprintf (meld_buf, VGL_WAHR_LOG_TXT, springanz);
#ifdef MITSIMULOG
	    write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
	 ...
	 case STORE_SENS:
	 ii     = act_item->u_el.u_xel.u_act.i_params[SENS_NR];
	 var_nr = act_item->u_el.u_xel.u_act.i_params[SENS_VAR_NR];
	 if ((var_nr < MAXAXISANZ) && (ii < MAXAXISANZ))
	 { /* Nummern ok */
	    sensorposvar[var_nr] = robotjoints[ii];
#if defined MIT_ANIM || defined MITSIMULOG
	    if (nichtevo)
	    { /* interaktiver Aufruf */
	       sprintf (meld_buf, STORE_SENS_LOG_TXT, ii, var_nr, 
			robotjoints[ii] * RADGRAD);
#ifdef MITSIMULOG
	       write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
	 ...
	 { /* Fehler */
	    sprintf (meld_buf, STORE_SENS_ERR_TXT, ii, var_nr, MAXAXISANZ);
	    fehler (SIMU_ROB_SIM, 13, NO_INFO, NO_INFO, meld_buf, SIMU_RZ_MELD);
	 ...
	 case SET_TIME:
	 var_nr = act_item->u_el.u_xel.u_act.i_params[ZEIT_VAR_NR]; 
	 if (var_nr < MAXZEITREG)
	 { /* RegisterNr ok */
	    zeitreg[var_nr] = zeit;
#if defined MIT_ANIM || defined MITSIMULOG
	    if (nichtevo)
	    { /* interaktiver Aufruf */
	       sprintf (meld_buf, SET_TIME_LOG_TXT, var_nr, zeit);
#ifdef MITSIMULOG
	       write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
	 ...
	 { /* Fehler */
	    sprintf (meld_buf, SET_TIME_ERR_TXT, var_nr, MAXZEITREG);
	    fehler (SIMU_ROB_SIM, 14, NO_INFO, NO_INFO, meld_buf, SIMU_RZ_MELD);
#ifdef MITSIMULOG                             /* SIMU_RZ_MELD bringt nur Text */
	    write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
	 } /* Fehler */
	 ...
	 if (var_nr < MAXZEITREG)
	 { /* RegisterNr ok */
	    zeitvar[var_nr] = zeit - zeitreg[var_nr];
#if defined MIT_ANIM || defined MITSIMULOG
	    if (nichtevo)
	    { /* interaktiver Aufruf */
	       sprintf (meld_buf, SET_TIME_LOG_TXT, var_nr, zeitvar[var_nr]);
#ifdef MITSIMULOG
	       write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
	 ...
	 { /* Fehler */
	    sprintf (meld_buf, SET_TIME_ERR_TXT, var_nr, MAXZEITREG-1);
	    fehler (SIMU_ROB_SIM, 15, NO_INFO, NO_INFO, meld_buf, SIMU_RZ_MELD);
#ifdef MITSIMULOG                             /* SIMU_RZ_MELD bringt nur Text */
	    write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
	 } /* Fehler */
                           ...
                           INT     achse)
{
   ...
   /* POS_VERGLEICH/SIMU */
   vwert = robotjoints[achse]*RADGRAD;
   switch (vgl_op)
   {
      case EQ:
      return (vwert == vergleichswert);
      ...
      default:
      fehler (SIMU_ROB_SIM, 17, NO_INFO, NO_INFO, BAD_POS_VGL_OP_TXT, 
	      SIMU_RZ_MELD);                /* SIMU_RZ_MELD bringt nur Text */
#ifdef MITSIMULOG
      write_to_simulog (BAD_POS_VGL_OP_TXT, CLOSE_LOG);
#endif /* MITSIMULOG */
                              ...
                              INT     achse)
{
   ...
   /* GESCHW_VERGLEICH/SIMU */
   vwert = (robotjoints[achse] - oldrobotjoints[achse]) * RADGRAD / bildtakt;
   switch (vgl_op)
   {
      case EQ:
      return (vwert == vergleichswert);
      ...
      default:
      fehler (SIMU_ROB_SIM, 18, NO_INFO, NO_INFO, BAD_GESCHW_VGL_OP_TXT, 
	      SIMU_RZ_MELD);                /* SIMU_RZ_MELD bringt nur Text */
#ifdef MITSIMULOG
      write_to_simulog (BAD_GESCHW_VGL_OP_TXT, CLOSE_LOG);
#endif /* MITSIMULOG */
      break;
   } /* CASE */
   return (FALSE);
                              ...
                              INT     achse)
{
   ...
   /* BESCHL_VERGLEICH/SIMU */
   vwert = ((robotjoints[achse] - oldrobotjoints[achse]) -
	    (oldrobotjoints[achse] - beschlrobotjoints[achse])) * 
           RADGRAD / bildtakt;
   switch (vgl_op)
   {
      case EQ:
      ...
      default:
      fehler (SIMU_ROB_SIM, 19, NO_INFO, NO_INFO, BAD_BESCHL_VGL_OP_TXT, 
	      SIMU_RZ_MELD);                /* SIMU_RZ_MELD bringt nur Text */
#ifdef MITSIMULOG
      write_to_simulog (BAD_BESCHL_VGL_OP_TXT, CLOSE_LOG);
#endif /* MITSIMULOG */
      break;
   } /* CASE */
   return (FALSE);
...
static void poscomp (INT  achse)
{
   ...
   /* POSCOMP/SIMU */
   if ((0 <= achse) && (achse < axisanz))
   { /* den Motor gibt's */
#if defined MIT_ANIM || defined MITSIMULOG
      if (nichtevo)
      { /* interaktiver Aufruf */
	 compstr (buf, act_item->u_el.u_xel.u_act.i_params[VERGL_OP]);
	 sprintf (meld_buf, ACHS_POS_VGL_LOG_TXT, achse, buf, 
		  act_item->u_el.u_xel.u_act.r_params[VERGL_WERT]);
#ifdef MITSIMULOG
	 write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
...
#endif /* MIT_ANIM || MITSIMULOG */
      if (pos_vergleich (act_item->u_el.u_xel.u_act.i_params[VERGL_OP],
			 act_item->u_el.u_xel.u_act.r_params[VERGL_WERT],
			 achse))
	 springanz = act_item->u_el.u_xel.u_act.i_params[AKT_ANZ];
   } /* den Motor gibt's */
...
static void geschwcomp (INT  achse)
{
   ...
   { /* den Motor gibt's */
      ...
      { /* interaktiver Aufruf */
	 compstr (buf, act_item->u_el.u_xel.u_act.i_params[VERGL_OP]);
	 sprintf (meld_buf, ACHS_GESCHW_VGL_LOGTXT, achse, buf, 
		  2 * act_item->u_el.u_xel.u_act.i_params[VERGL_WERT]);
#ifdef MITSIMULOG
	 write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
...
#endif /* MIT_ANIM || MITSIMULOG */
      if (geschw_vergleich (act_item->u_el.u_xel.u_act.i_params[VERGL_OP],
			    act_item->u_el.u_xel.u_act.i_params[VERGL_WERT],
			    achse))
	 springanz = act_item->u_el.u_xel.u_act.i_params[AKT_ANZ];
   }  /* den Motor gibt's */
   else
...
static void beschlcomp (INT  achse)
{
  ...
  { /* den Motor gibt's */
      ...
      { /* interaktiver Aufruf */
	 compstr (buf, act_item->u_el.u_xel.u_act.i_params[VERGL_OP]);
	 sprintf (meld_buf, ACHS_BESCHL_VGL_LOGTXT, achse, buf, 
		  2 * act_item->u_el.u_xel.u_act.i_params[VERGL_WERT]);
#ifdef MITSIMULOG
	 write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
...
#endif /* MIT_ANIM || MITSIMULOG */
      if (beschl_vergleich (act_item->u_el.u_xel.u_act.i_params[VERGL_OP],
			    act_item->u_el.u_xel.u_act.i_params[VERGL_WERT],
			    achse))
	 springanz = act_item->u_el.u_xel.u_act.i_params[AKT_ANZ];
  }  /* den Motor gibt's */
...
static void zeitcomp (void)
{
   ...
   /* ZEITCOMP/SIMU */
   vgl_wert     = act_item->u_el.u_xel.u_act.r_params[VERGL_WERT];
   zeit_var_idx = act_item->u_el.u_xel.u_act.i_params[ZEIT_VAR_NR];
   if (zeit_var_idx < MAXZEITREG)
   { /* RegisterNr ok */
      switch (act_item->u_el.u_xel.u_act.i_params[VERGL_OP])
      {
	 case EQ:
	 zeit_vergleich = vgl_wert == zeitvar[zeit_var_idx];
	 break;
	 ...
	 case GT:
	 zeit_vergleich = zeitvar[zeit_var_idx] > vgl_wert;
	 break;
	 ...
	 default:
	 fehler (SIMU_ROB_SIM, 20, NO_INFO, NO_INFO, BAD_ZEIT_VGL_OP_TXT, 
		 SIMU_RZ_MELD);               /* SIMU_RZ_MELD bringt nur Text */
#ifdef MITSIMULOG
	 write_to_simulog (BAD_ZEIT_VGL_OP_TXT, CLOSE_LOG);
#endif /* MITSIMULOG */
	 zeit_vergleich = FALSE;
	 break;
      } /* CASE */
#if defined MIT_ANIM || defined MITSIMULOG
      if (nichtevo)
      { /* interaktiver Aufruf */
	 compstr (buf, act_item->u_el.u_xel.u_act.i_params[VERGL_OP]);
	 sprintf (meld_buf, ZEIT_VGL_LOG_TXT, zeit_var_idx, 
		  zeitvar[zeit_var_idx], buf, vgl_wert);
#ifdef MITSIMULOG
	 write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
...
#endif /* MIT_ANIM || MITSIMULOG */
      if (zeit_vergleich)
	 springanz = act_item->u_el.u_xel.u_act.i_params[AKT_ANZL];
   } /* RegisterNr ok */
   else
   { /* Falsche RegisterNr */
      sprintf (meld_buf, BAD_ZEIT_REG_NR_TXT, zeit_var_idx, MAXAXISANZ-1);
      fehler (SIMU_ROB_SIM, 21, NO_INFO, NO_INFO, meld_buf, SIMU_RZ_MELD);
#ifdef MITSIMULOG                /* SIMU_RZ_MELD bringt nur Text */
      write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
   } /* Falsche RegisterNr */
...
{
   char  buf [4];
   INT   vgl_wert_idx, sens_var_idx;
   BOOL  sens_vergleich;
  
   /* SENSCOMP/SIMU */
   vgl_wert_idx = act_item->u_el.u_xel.u_act.i_params[SENS_NR];
   sens_var_idx = act_item->u_el.u_xel.u_act.i_params[SENS_VAR_NR];
   if ((sens_var_idx < MAXAXISANZ) && (vgl_wert_idx < MAXAXISANZ))
   { /* SensorNr und SensorvariablenNr o.k. */
      switch (act_item->u_el.u_xel.u_act.i_params[VGL_OP])
      {
	 case EQ:
	 sens_vergleich = sensorposvar[sens_var_idx] == robotjoints[vgl_wert_idx];
	 break;
	 ...
	 default:
	 fehler (SIMU_ROB_SIM, 22, NO_INFO, NO_INFO, BAD_SENS_VGL_OP_TXT, 
		 SIMU_RZ_MELD);               /* SIMU_RZ_MELD bringt nur Text */
#ifdef MITSIMULOG
	 write_to_simulog (BAD_SENS_VGL_OP_TXT, CLOSE_LOG);
#endif /* MITSIMULOG */
	 sens_vergleich = FALSE;
	 break;
      } /* CASE */
#if defined MIT_ANIM || defined MITSIMULOG
      if (nichtevo)
      { /* interaktiver Aufruf */
	 compstr (buf, act_item->u_el.u_xel.u_act.i_params[VERGL_OP]);
	 sprintf (meld_buf, SENS_VGL_LOG_TXT, vgl_wert_idx, 
		  sens_var_idx, sensorposvar[sens_var_idx] * RADGRAD, buf,
		  robotjoints[vgl_wert_idx] * RADGRAD);
#ifdef MITSIMULOG
	 write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
...
#endif /* MIT_ANIM || MITSIMULOG */
      if (sens_vergleich)
	 springanz = act_item->u_el.u_xel.u_act.i_params[AKT_ANZAHL];
   } /* SensorNr und SensorvariablenNr o.k. */
   else
   { /* Fehlerhafte SensorNr und/oder  SensorvariablenNr */
      sprintf (meld_buf, BAD_SENS_REG_NR_TXT, vgl_wert_idx, sens_var_idx, 
	       MAXAXISANZ-1);
      fehler (SIMU_ROB_SIM, 23, NO_INFO, NO_INFO, meld_buf, SIMU_RZ_MELD);
#ifdef MITSIMULOG                /* SIMU_RZ_MELD bringt nur Text */
      write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
   } /* Fehlerhafte SensorNr und/oder  SensorvariablenNr */
...
static void abstandsvergl (INT  switch_1)
{
   STR_TYPE  text1, text2;
   INT       motor;
   VECTOR    anfpos, strahlpos;
   DOUBLE    distanz;
  
   /* ABSTANDSVERGL/SIMU */
   if ((mitr500))
      zeigr500 (robotr500, robotjoints, pm, &(p[1]), MITSTRAHL);
   else
      ktrwrobmod (robotjoints, axisanz, OHNEORI);
   distanz = strahlabstand (armfeld[axisanz], &anfpos, &strahlpos, switch_1);
   motor   = act_item->u_el.u_xel.u_act.i_params[MOTOR_NR];
#ifdef MIT_ANIM
   if (nichtevo)
   {
      _setcolor (WHITE);
      drawline (&anfpos, &strahlpos, pm);
   }
#endif /* MIT_ANIM */
#if defined MIT_ANIM || defined MITSIMULOG
   if (nichtevo)
   { /* interaktiver Aufruf */
      sprintf (text1, ABST_ACHSE_TXT, motor);
      if (distanz < 0.0)
      {
	 strcpy (text1, ABST_KEIN_KONTAKT_TXT);
	 strcpy (text2, "");
      }
      else
	 sprintf (text2, " d=%4.1f", distanz);
      strcpy (meld_buf, ABST_VGL_LOG_TXT);
      if (switch_1 == GREIFEROUT)
	 strcat (meld_buf, ABST_AUSSEN_TXT);
      strcat (meld_buf, ": ");
      strcat (meld_buf, text1);
      strcat (meld_buf, text2);
#ifdef MITSIMULOG
      write_to_simulog (meld_buf, CLOSE_LOG);
#endif /* MITSIMULOG */
#ifdef MIT_ANIM
...
#endif /* MIT_ANIM || MITSIMULOG */
   if (distanz > 0.0)
   { /* Vergl_Wert = AchsenNr, Aktions_Anz = Prozent */
      steuerfeld[motor].rampe = 
	 fabs(aktufeld[motor] * 
	      act_item->u_el.u_xel.u_act.r_params[PROZENT] / 100.0);
      steuerfeld[motor].spannung = 0.0;
   } /* Vergl_Wert = AchsenNr, Aktions_Anz = Prozent */
} /* abstandsvergl */ 
